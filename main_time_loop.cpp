...
int main()
{
    ...
    writeResidualHeader("residual_RK4.dat");

    // ============================================================
    // Write initial solution
    // ============================================================

    writeTecplot(field, grid, "solution_RK4_0.dat");
    output_counter = 1;
    ...
    while (solver.time < solver.final_time)
    {
        ...
        appendResidual("residual_RK4.dat", solver.step, solver.time, residual);

        if (solver.time >= next_output_time || solver.time >= solver.final_time)
        {
            std::string filename = "solution_RK4_" + std::to_string(output_counter) + ".dat";
